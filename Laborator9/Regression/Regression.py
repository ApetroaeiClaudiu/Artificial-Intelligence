...
class Regression:
    ...
    def fitBatch(self, x, y,className, learningRate = 0.1, noEpochs = 1000):
    ...
    def fitSimple(self, x, y, className,learningRate = 0.001, noEpochs = 1000):
        ...
        for epoch in range(noEpochs):
            for i in range(len(x)): # for each sample from the training data
                ycomputed = self.sigmoid(self.eval(x[i], self.coef_))     # estimate the output
